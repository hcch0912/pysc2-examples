...
def learn(env,
  ...
  act_z, train_z, update_target_z, debug_z = deepq.build_train(
      make_obs_ph=make_obs_ph,
      q_func=q_func,
      num_actions=num_actions,
      optimizer=tf.train.AdamOptimizer(learning_rate=lr),
  ...
  if prioritized_replay:
    ...
    replay_buffer_z = PrioritizedReplayBuffer(buffer_size, alpha=prioritized_replay_alpha)
    ...
    beta_schedule_y = LinearSchedule(prioritized_replay_beta_iters,
                                     initial_p=prioritized_replay_beta0,
                                     final_p=1.0)
    beta_schedule_z = LinearSchedule(prioritized_replay_beta_iters,
                                     initial_p=prioritized_replay_beta0,
                                     final_p=1.0)
  else:
    replay_buffer_x = ReplayBuffer(buffer_size)
    replay_buffer_y = ReplayBuffer(buffer_size)
    replay_buffer_z = ReplayBuffer(buffer_size)

    beta_schedule_x = None
    beta_schedule_y = None
    beta_schedule_z = None
  ...
  player_z, player_y, player_x = (player_relative == _PLAYER_FRIENDLY).nonzero()
  player = [int(player_x.mean()), int(player_y.mean()), int(player_z.mean())]
  ...
  with tempfile.TemporaryDirectory() as td:
    ...
    for t in range(max_timesteps):
      ...
      action_z = act_z(np.array(screen)[None], update_eps=update_eps, **kwargs)[0]
      ...
      coord = [action_x, action_y, action_z]
      ...
      replay_buffer_z.add(screen, action_z, rew, new_screen, float(done))
      ...
      if t > learning_starts and t % train_freq == 0:
        ...
        if prioritized_replay:
          ...
          experience_z = replay_buffer_z.sample(batch_size, beta=beta_schedule_z.value(t))
          (obses_t_z, actions_z, rewards_z, obses_tp1_z, dones_z, weights_z, batch_idxes_z) = experience_z
        else:
          ...
          obses_t_z, actions_z, rewards_z, obses_tp1_z, dones_z = replay_buffer_z.sample(batch_size)
          weights_z, batch_idxes_z = np.ones_like(rewards_z), None
        ...
        td_errors_z = train_x(obses_t_z, actions_z, rewards_z, obses_tp1_z, dones_z, weights_z)


        if prioritized_replay:
          new_priorities_x = np.abs(td_errors_x) + prioritized_replay_eps
          new_priorities_y = np.abs(td_errors_y) + prioritized_replay_eps
          new_priorities_z = np.abs(td_errors_z) + prioritized_replay_eps
          ...
          replay_buffer_z.update_priorities(batch_idxes_z, new_priorities_z)
  ...
  return ActWrapper(act_x), ActWrapper(act_y), ActWrapper(act_z)
